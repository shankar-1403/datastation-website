...
export default function FortyKMsmePage() {
  const { shopifyCartBaseUrl, shopifyFortyMSMEVariantId, shopifyProductFallbackUrl } = useLoaderData();

  const handleBuy = () => {
    if (
      openShopifyCheckout({
        cartBaseUrl: shopifyCartBaseUrl,
        variantId: shopifyFortyMSMEVariantId,
      })
    ) {
      return;
    }
  ...
  const item = getProductPrice("msme-40k");
  ...
  return (
    <div className="max-w-full overflow-x-hidden bg-white px-4 pb-20 pt-20 sm:px-6 sm:pt-24 lg:px-8 lg:pt-30 min-[1100px]:px-13">
      <div className="mb-8 space-y-4 text-sm leading-relaxed">
        <span className="font-bold text-[#5c5c5c]">Overview</span>
        <svg
          viewBox="0 0 1000 30"
