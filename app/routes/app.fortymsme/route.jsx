...
const FORTYMSME_HANDLE = DATA_PRODUCTS.find((p) => p.id === "msme-40k")?.handle ?? "40-000-msme-database";
...
export const loader = async ({ request }) => {
  ...
  return {
    ...
    /** Required for best checkout UX. Example: PUBLIC_SHOPIFY_FORTYMSME_VARIANT_ID=1234567890 */
    shopifyFortyMSMEVariantId: process.env.PUBLIC_SHOPIFY_FORTYMSME_VARIANT_ID || "",
    ...
    shopifyProductFallbackUrl:
      process.env.PUBLIC_SHOPIFY_FORTYMSME_PRODUCT_URL ||
      productUrl(storefrontBase, FORTYMSME_HANDLE),
  };
};
...
export default function FortyKMsmePage() {
  ...
  const handleCheckoutRedirect = () => {
    ...
    if (shopifyFortymsmeVariantId) {
      // https://shopify.dev/docs/apps/build/checkout/create-cart-permalinks
      const cartUrl = new URL(`${base}/products/${shopifyFortymsmeVariantId}:1`);
    ...
    <div className="max-w-full overflow-x-hidden pt-30 pb-20 px-4 sm:px-6 lg:px-8 min-[1100px]:px-13 bg-white">
      <div className="mb-8 space-y-4 text-sm leading-relaxed">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:items-start lg:gap-10">
          ...
          <div className="min-w-0">
            <h1 className="font-heading text-3xl font-bold leading-[1.1] tracking-tight text-[#ed501f] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl">40,000 MSME Database</h1>
            <p className="mt-4 text-base leading-relaxed text-[#5c5c5c] sm:mt-5 sm:text-lg">
              The 40,000 MSME Database provides access to a structured dataset of MSME companies across India.
            </p>
            <p className="mt-4 text-base leading-relaxed text-[#5c5c5c] sm:mt-5 sm:text-lg">
      ...
      <DatabaseCarousel excludeProductId="msme-40k" />
    </div>
  );
}
