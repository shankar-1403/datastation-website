...
const MINERALS_METALS_HANDLE = DATA_PRODUCTS.find((p) => p.id === "minerals-metals")?.handle ?? "minerals-metals-database";
...
export const loader = async ({ request }) => {
  ...
  return {
    ...
    /** Example: PUBLIC_SHOPIFY_MINERALS_METALS_VARIANT_ID=1234567890 */
    shopifyMineralsMetalsVariantId: process.env.PUBLIC_SHOPIFY_MINERALS_METALS_VARIANT_ID || "",
    shopifyProductFallbackUrl:
      process.env.PUBLIC_SHOPIFY_MINERALS_METALS_PRODUCT_URL ||
      productUrl(storefrontBase, MINERALS_METALS_HANDLE),
  };
};

export default function MineralsMetalsPage() {
  const { shopifyCartBaseUrl, shopifyMineralsMetalsVariantId, shopifyProductFallbackUrl } = useLoaderData();

  const handleBuy = () => {
    if (
      openShopifyCheckout({
        cartBaseUrl: shopifyCartBaseUrl,
        variantId: shopifyMineralsMetalsVariantId,
      })
    ) {
      return;
    }
  ...
  const mediaData = [
    { type: "image", src: "/minerals_metals.webp" },
  ...
  const item = getProductPrice("minerals-metals");

  const breadcrumbItems = [
    { label: "Home", href: "/app" },
    { label: "Products", href: "/app/products" },
    { label: "12,900 Minerals & Metals Database" },
  ];
    ...
    <div className="max-w-full overflow-x-hidden bg-white px-4 pb-20 pt-20 sm:px-6 sm:pt-24 lg:px-8 lg:pt-30 min-[1100px]:px-13">
      <div className="mb-8 space-y-4 text-sm leading-relaxed">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:items-start lg:gap-10">
          ...
          <div className="min-w-0">
            <h1 className="font-heading font-bold leading-[1.1] tracking-tight text-[#ed501f] sm:text-4xl md:text-3xl lg:text-5xl">12,900 Minerals & Metals Database</h1>
            <p className="mt-4 text-base leading-relaxed text-[#5c5c5c] sm:mt-5 sm:text-lg">The 12,900 Minerals & Metals Database provides access to a structured dataset of minerals and metals companies across India.</p>
            <p className="mt-4 text-base leading-relaxed text-[#5c5c5c] sm:mt-5 sm:text-lg">This dataset is designed for professionals who want to connect with companies across mining, metal processing, and raw material supply segments for sales, sourcing, or outreach.</p>
      ...
      <div className="pt-10">
        ...
        <DatabaseCarousel excludeProductId="minerals-metals" />
      </div>
    </div>
  );
}
